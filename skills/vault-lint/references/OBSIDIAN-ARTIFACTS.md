...
## Hub Page (Structured Note + Folder Index)

A human-readable entry point into a project or topic cluster, and the folder's **per-folder index** (OKF progressive disclosure). The global Wiki Index is optimised for LLM routing; hub pages are optimised for humans — richer, contextual, scannably structured — *and* they hold the generated listing the agent drills into when the global index delegates a large folder.

One file, two zones: a **curated** top half a human writes/edits, and a **generated `## Index`** section that vault-lint owns and regenerates from the global Wiki Index. Never hand-edit the `## Index` block — it will be overwritten.
...
<!-- BEGIN GENERATED INDEX — maintained by vault-lint, do not edit by hand -->
## Index

_Every note in this folder. Regenerated by vault-lint._

- [[note-filename]] · `type` · #tag — one-sentence description (YYYY-MM-DD)
<!-- END GENERATED INDEX -->
```

**Relationship to the global Wiki Index:** the global index stays canonical. When a folder grows past ~20–30 notes, its global entry collapses to a pointer (`PROJECT NAME — N notes → [[00 - X Overview]]`) and the detail lives in this `## Index` block. Small folders need no hub page; the global index lists them directly.
